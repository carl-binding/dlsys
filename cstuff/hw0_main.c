...
static t_tensor normalize( const t_tensor Z) {
  ...
  t_tensor sum_rows_Z = t_sum( Z, row_axis);  // sum over rows
...
// in-situ division
static t_tensor t_div_scalar( const t_tensor t, const double denom) {
...
static t_tensor t_mul_scalar( const t_tensor t, const double m) {
