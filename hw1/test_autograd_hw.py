...
def test_transpose_forward():
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor([[[1.95]], [[2.7]], [[3.75]]]), axes=(0, 1, 2)).numpy(),
        np.array([[[1.95]], [[2.7]], [[3.75]]]),
    )
    np.testing.assert_allclose(
        ndl.transpose(
            ndl.Tensor([[[[0.95]]], [[[2.55]]], [[[0.45]]]]), axes=(0, 1, 2, 3)
        ).numpy(),
    ...
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor([[[2.45]], [[3.5]], [[0.9]]]), axes=(1, 0, 2)).numpy(),
...
def gradient_check(f, *args, tol=1e-6, backward=False, **kwargs):
    ...
    for i in range( len(args)):
      print( computed_grads[i])
      print( numerical_grads[i])
...
def test_transpose_backward():
    gradient_check(ndl.transpose, ndl.Tensor(np.random.randn(3, 5, 4)))
    gradient_check(ndl.transpose, ndl.Tensor(np.random.randn(3, 5, 4)), axes=(0, 1, 2))
    gradient_check(ndl.transpose, ndl.Tensor(np.random.randn(3, 5, 4)), axes=(2, 1, 0))
    gradient_check(ndl.transpose, ndl.Tensor(np.random.randn(3, 5, 4)), axes=(1, 0, 2))
    gradient_check(ndl.transpose, ndl.Tensor(np.random.randn(3, 5, 4)), axes=(0, 2, 1))
...
def test_nn_epoch_ndl():
    ...
    W1, W2 = nn_epoch(X, y, W1, W2, lr=0.2, batch=100, debug=True)
    np.testing.assert_allclose(
